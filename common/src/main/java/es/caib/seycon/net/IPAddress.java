...
public class IPAddress 
{
  ...
  {
    if (s == null) throw new InvalidIPException (Messages.getString("IPAddress.EmptyAdress")); //$NON-NLS-1$
    StringTokenizer tokenizer = new StringTokenizer (s, ".", false); //$NON-NLS-1$
    int i = 0;
    while (tokenizer.hasMoreTokens()) 
    {
      String token = tokenizer.nextToken();
      if ( i >= 4 ) throw new InvalidIPException ( Messages.getString("IPAddress.MoreThan4Digits")); //$NON-NLS-1$
      try {
        int x = new Integer (token).intValue ();
        if ( x > 255 || x < 0 )
          throw new InvalidIPException (String.format(Messages.getString("IPAddress.WrongNumber"), x)); //$NON-NLS-1$
        ip [ i ++ ] = x;
    ...
    throws InvalidIPException 
  {
    ...
    {
      if ( ( ip [ i ] & mask [ i ] ) != ip [ i ] )
        throw new InvalidIPException (String.format(Messages.getString("IPAddress.WrongMask"),s)); //$NON-NLS-1$
    }
  }
  ...
  {
    String s = String.format("%s.%s.%s.%s,",ip[0],ip[1],ip[2],ip[3]); //$NON-NLS-1$
    if (xarxa) 
      s = s+"/"+String.format("%s.%s.%s.%s,",mask[0],mask[1],mask[2],mask[3]); //$NON-NLS-1$ //$NON-NLS-2$
    return s;
  }
