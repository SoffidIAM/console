...
public class UsuariServiceImpl extends
	...
	@Override
	protected Collection<Usuari> handleFindUserByJsonQuery(String query)
			throws Exception {
		ClassConfig config = Configuration.getClassConfig(com.soffid.iam.api.User.class);

		AbstractExpression expr = ExpressionParser.parse(query);
		HQLQuery hql = expr.generateHSQLString(Usuari.class);
		...
		LinkedList<Usuari> result = new LinkedList<Usuari>();
		for (UserEntity ue: getUserEntityDao().query(hql.toString(), paramArray))
		{
			Usuari u = getUserEntityDao().toUsuari(ue);
			if (! hql.isNonHQLAttributeUsed() || expr.evaluate(com.soffid.iam.api.User.toUser(u)))
			{
