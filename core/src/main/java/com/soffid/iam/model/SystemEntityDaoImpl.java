...
public class SystemEntityDaoImpl extends com.soffid.iam.model.SystemEntityDaoBase {
	...
	public void remove(com.soffid.iam.model.SystemEntity dispatcherEntity) throws RuntimeException {
		try
		{
			...
			getSession().createQuery("delete from com.soffid.iam.model.UserAccountEntity as uac "
			...
			getSession().createQuery("delete from com.soffid.iam.model.AccountPasswordEntity where account.id in "
				...
				.executeUpdate();
	 		// Remove account acl
			getSession().createQuery("delete from com.soffid.iam.model.AccountAccessEntity where account.id in "
					+ "(select acc.id from com.soffid.iam.model.AccountEntity as acc where acc.system.id=:system)")
				.setLong("system", dispatcherEntity.getId())
				.executeUpdate();
	 		// Remove account services
			getSession().createQuery("delete from com.soffid.iam.model.HostServiceEntity where account.id in "
					+ "(select acc.id from com.soffid.iam.model.AccountEntity as acc where acc.system.id=:system)")
				.setLong("system", dispatcherEntity.getId())
				.executeUpdate();
	 		// Remove accounts
