 ...
 */
public class DispatcherServiceImpl extends com.soffid.iam.service.DispatcherServiceBase {
     ...
     */
    protected com.soffid.iam.api.System handleCreate(com.soffid.iam.api.System dispatcher) throws java.lang.Exception {
		...
		SystemEntity dispatchersSameCode = getSystemEntityDao().findByName(dispatcher.getName());
		if(dispatchersSameCode != null)
			throw new SeyconException(String.format(Messages.getString("DipatcherServiceImpl.CodeDispatcherExists"), dispatcher.getName())); 
        
        SystemEntity dispatcherEntity = getSystemEntityDao().systemToEntity(dispatcher);
	...
	private void updateAutomaticTasks(com.soffid.iam.api.System dispatcher, boolean remove) throws InternalErrorException {
	...
	private void updateAutomaticTasks(String handler, String description, com.soffid.iam.api.System dispatcher, boolean remove) throws InternalErrorException {
     ...
     */
    protected com.soffid.iam.api.System handleUpdate(com.soffid.iam.api.System dispatcher) throws java.lang.Exception {
        // Obtenim el anterior per comparar els grups i els tipus d'usuari
        SystemEntity entityOld = getSystemEntityDao().findByName(dispatcher.getName());
    ...
    private void updateTipusAndGrups(com.soffid.iam.api.System dispatcher, SystemEntity entity) throws InternalErrorException, Exception {
        updateTipus(dispatcher, entity);
        updateGrups(dispatcher, entity);
    }

    private void updateTipus(com.soffid.iam.api.System dispatcher, SystemEntity entity) throws InternalErrorException, Exception {
        ...
        for (Iterator<UserTypeSystemEntity> it = entity.getUserType().iterator(); it.hasNext(); ) {
            ...
            if (!found) {
                ...
                Collection<User> usuaris = usuService.findUserByCriteria("", "", "", "", "", "", "", "", "", "", td.getUserType().getName(), "", "", "", "", "", "", "", false);
        ...
        for (UserTypeEntity tu : tipusUsuariToGenerateAccounts) {
            UserService usuService = getUserService();
            Collection<User> usuaris = usuService.findUserByCriteria("", "", "", "", "", "", "", "", "", "", tu.getName(), "", "", "", "", "", "", "", false);
            accService = getAccountService();
            long l = usuaris.size();
            int i = 0;
            for (User usuari : usuaris) {
    ...
    private void updateGrups(com.soffid.iam.api.System dispatcher, SystemEntity entity) throws InternalErrorException, Exception {
        GroupEntityDao grupDao = getGroupEntityDao();
        com.soffid.iam.service.AccountService accService = getAccountService();
        String[] grups = dispatcher.getGroups() == null ? new String[0] : dispatcher.getGroups().split(","); //$NON-NLS-1$
	 ...
	 */
	protected void handleDelete(com.soffid.iam.api.System dispatcher) throws java.lang.Exception {
		SystemEntity dispatcherEntity = getSystemEntityDao().findByName(dispatcher.getName());
    ...
    protected com.soffid.iam.api.System handleFindDispatcherByName(String codi) throws Exception {
        ...
        if (dispatcherEntity != null) {
            com.soffid.iam.api.System dispatcher = getSystemEntityDao().toSystem(dispatcherEntity);
            return dispatcher;
        }
        return null;
    }
    ...
    protected Collection<com.soffid.iam.api.System> handleFindDispatchersByFilter(String codi, String nomCla, String url, String basRol, String segur, Boolean actiu) throws Exception {
        ...
        if (dispatchers != null)
        {
        	...
        	{
        		return getSystemEntityDao().toSystemList(dispatchers).subList(0, limitResults);
        	}
    ...
    protected AccessControl handleCreate(AccessControl controlAcces) throws Exception {
        if (AutoritzacionsUsuari.canCreateAccessControlAgent()) {
            AccessControlEntity entity = getAccessControlEntityDao().accessControlToEntity(controlAcces);
    ...
    protected AccessControl handleUpdate(AccessControl controlAcces) throws Exception {
    ...
    protected Collection<AccessControl> handleFindAccessControlByDispatcherName(String codiAgent) throws Exception {
    ...
    @Override
    protected UserTypeDispatcher handleCreate(UserTypeDispatcher tipusUsuari) throws Exception {
        return null;
    }

    @Override
    protected void handleDelete(UserTypeDispatcher tipusUsuari) throws Exception {
    }

    @Override
    protected SystemGroup handleCreate(SystemGroup grupDispatcher) throws Exception {
        return null;
    }
    ...
    @Override
    protected UserTypeDispatcher handleUpdate(UserTypeDispatcher tipusUsuari) throws Exception {
        return null;
    }

    @Override
    protected SystemGroup handleUpdate(SystemGroup grupDispatcher) throws Exception {
        return null;
    }

    @Override
    protected Collection<AccessControl> handleGetAccessControl(com.soffid.iam.api.System agent) throws Exception {
        Collection<AccessControlEntity> cace = getAccessControlEntityDao().findByAgentCode(agent.getName());
        return getAccessControlEntityDao().toAccessControlList(cace);
    }

    @Override
    protected Collection<SystemGroup> handleGetDispatcherGroups(com.soffid.iam.api.System agent) throws Exception {
        Collection<SystemGroupEntity> grupsE = getSystemGroupEntityDao().findBySystem(agent.getName());
        return getSystemGroupEntityDao().toSystemGroupList(grupsE);
    }

    @Override
    protected Collection<UserTypeDispatcher> handleGetDispatcherUserTypes(com.soffid.iam.api.System agent) throws Exception {
        Collection<UserTypeSystemEntity> tipusUsuariE = getUserTypeSystemEntityDao().findBySystem(agent.getName());
        return getUserTypeSystemEntityDao().toUserTypeDispatcherList(tipusUsuariE);
    ...
    protected boolean handleIsUserAllowed(com.soffid.iam.api.System dispatcher, String user) throws Exception {
        ...
        if (de.getRoleBased().equals("S")) { //$NON-NLS-1$
        	Collection<RoleGrant> grants = getApplicationService().findEffectiveRoleGrantByUser(ue.getId());
        	for (RoleGrant grant : grants) {
                if (grant.getSystem().equals(de.getName())) return true;
            }
        	return false;
        }
        return true;
    ...
    protected Collection<com.soffid.iam.api.System> handleFindAllActiveDispatchers() throws Exception {
		return getSystemEntityDao().toSystemList(getSystemEntityDao().findActives());
	}
	 ...
	 */
	@Override
    protected Collection<com.soffid.iam.api.ReplicaDatabase> handleFindAllDatabases() throws Exception {
	 ...
	 */
	@Override
    protected com.soffid.iam.api.ReplicaDatabase handleUpdate(com.soffid.iam.api.ReplicaDatabase database) throws Exception {
	 ...
	 */
	@Override
    protected com.soffid.iam.api.ReplicaDatabase handleCreate(com.soffid.iam.api.ReplicaDatabase database) throws Exception {
		ReplicaDatabaseEntityDao dao = getReplicaDatabaseEntityDao();
		ReplicaDatabaseEntity entity = dao.replicaDatabaseToEntity(database);
	...
	private void updateReplicaParameter () throws InternalErrorException
	{
		...
		Configuration config = configSvc.findParameterByNameAndNetworkName("soffid.replica.enabled", null); //$NON-NLS-1$
		if (config == null)
		{
	 ...
	 */
	@Override
    protected void handleDelete(com.soffid.iam.api.ReplicaDatabase database) throws Exception {
	...
	protected void updateSeyconServerList () throws InternalErrorException
	{
		...
		ConfigurationService configSvc = getConfigurationService();
		Configuration config = configSvc.findParameterByNameAndNetworkName("seycon.server.list", null); //$NON-NLS-1$
	 ...
	 */
	@Override
    protected com.soffid.iam.api.ReplicaDatabase handleFindReplicaDatabase(Long id) throws Exception {
