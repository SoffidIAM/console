...
public class TenantDataManager {
	...
	String[] hints = new String[] {
				...
				"JBPM_COMMENT", "WHERE TOKEN_ IN (SELECT ID_ FROM JBPM_TOKEN ",
									  "WHERE PROCESSINSTANCE_ IN "
									  + "(SELECT JBPM_MODULEINSTANCE.PROCESSINSTANCE_ FROM JBPM_MODULEINSTANCE "
									  +   "WHERE JBPM_MODULEINSTANCE.TENANT_=${tenantId}))",
				"JBPM_LOG", "WHERE TOKEN_ IN (SELECT ID_ FROM JBPM_TOKEN ",
				  "WHERE PROCESSINSTANCE_ IN "
				  + "(SELECT JBPM_MODULEINSTANCE.PROCESSINSTANCE_ FROM JBPM_MODULEINSTANCE "
				  +   "WHERE JBPM_MODULEINSTANCE.TENANT_=${tenantId}))",
