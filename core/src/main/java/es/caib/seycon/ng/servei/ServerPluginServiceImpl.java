 ...
 */
public class ServerPluginServiceImpl extends es.caib.seycon.ng.servei.ServerPluginServiceBase {
    ...
    protected void handleDeployPlugin(byte b[]) throws java.lang.Exception {
    	...
    	if (spp.getPlugin() == null)
    		throw new InternalErrorException (Messages.getString("ServerPluginServiceImpl.NotAPluginFile")); //$NON-NLS-1$
     ...
     *      boolean)
     */
    protected void handleEnablePlugin(es.caib.seycon.ng.comu.ServerPlugin plugin, boolean status)
            throws java.lang.Exception {
     ...
     */
    protected es.caib.seycon.ng.comu.AgentDescriptor handleGetAgentDescriptor(
            java.lang.String className) throws java.lang.Exception {
    ...
    {
    	Configuracio config = getConfiguracioService().findParametreByCodiAndCodiXarxa("plugin.timestamp", null); //$NON-NLS-1$
    	if (config == null)
    	{
    		config = new Configuracio();
    		config.setCodi("plugin.timestamp"); //$NON-NLS-1$
    		config.setCodiXarxa(null);
    		config.setDescripcio("Autogenerated value. Do not modify"); //$NON-NLS-1$
    		config.setValor(Long.toString(System.currentTimeMillis()));
