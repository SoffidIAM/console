...
public class OrderByExpression extends AbstractExpression {
	...
	public void generateHSQLString(HQLQuery query) throws EvalException {
		...
		if (orderBy != null) {
			if (query.getWhereString().length() > 0 && isOracleWorkaround()) {
				HQLQuery sortQuery = new HQLQuery(query.getClassConfig());
				sortQuery.setRootObject(ROOT_OBJECT_NAME);
				...
				for (String s: orderBy) {
					try {
						if (attName && (s.equalsIgnoreCase("asc") || s.equalsIgnoreCase("desc"))) {
							...
							attName = true;
							EvaluationContext ctx = generateHQLStringReference(sortQuery, s);
				...
				query.setOrderBy(o);
				query.setOuterJoinString(sortQuery.getJoinString());
			} else {				
				query.setRootObject(ROOT_OBJECT_NAME); // Oracle query without where condition but with order by condition
				LinkedList<String> o = new LinkedList<String>();
				boolean attName = false;
				for (String s: orderBy) {
					try {
