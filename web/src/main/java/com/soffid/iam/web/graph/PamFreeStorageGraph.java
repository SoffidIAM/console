...
@WebServlet(urlPatterns="/graph/pam_storage_free")
public class PamFreeStorageGraph extends HttpServlet {
	...
	protected HashMap<String, Object> getStats()
	{
		try {
			...
			Stats stats = statsService.findStats("PAM_STORAGE_FREE", c.getTime(), new Date(), 60*24);
			...
			for ( String serie: k)
			{
				...
				{
					datasetdata.add(value.getMin()/1024/1024);
				}
			...
			title.put("text", "Free storage by PAM storage server (MB)");
