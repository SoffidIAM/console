...
public class PermissionsHandler implements PersistenceHandler , FinderHandler {

	private TenantService ejb;

	public PermissionsHandler() throws NamingException, CreateException {
		super();
		ejb = EJBLocator.getTenantService();
	}

	public static StringInfo newPermission () throws InternalErrorException, NamingException, CreateException
	{
	...
	public Collection find(DataContext arg0) throws Exception {
		...
		for (String s: ejb.getDisabledPermissions(tenant))
		{
			StringInfo d = new StringInfo();
			d.setNewValue(s);
			d.setOldValue(s);
	...
	public void doDelete(DataContext ctx) throws Exception {
		...
		if (info.oldValue != null && ! info.oldValue.isEmpty())
			ejb.enablePermission(tenant, info.oldValue);
	}
	...
	public void doInsert(DataContext ctx) throws Exception {
		...
		if (info.newValue != null && ! info.newValue.isEmpty())
			ejb.disablePermission(tenant, info.newValue);
	}
