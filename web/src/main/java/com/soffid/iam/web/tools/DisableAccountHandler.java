...
public class DisableAccountHandler extends FrameHandler {
	...
	public DisableAccountHandler() throws InternalErrorException {
		super();
	}
	...
	public void next(Event event) throws Exception {
		...
		if (wizard.getSelected() == 3) {
			...
			currentProcess = EJBLocator.getAccountService().disableAccountsPreview(query, rules, result);
		...
		if (wizard.getSelected() == 4) {
			String query = (String) getModel().getJXPathContext().getVariables().getVariable("query");
			result = new LinkedList<>();
			currentProcess = EJBLocator.getAccountService().disableAccounts(query, rules);
	...
	private void addNewRow() {
		...
		msg.setText(Labels.getLabel("tools.disable-accounts.message"));
		final Textbox sub = new Textbox();
		sub.setVisible(false);
		sub.setText(Labels.getLabel("tools.disable-accounts.defaultSubject"));
		final Textbox to = new Textbox();
		to.setVisible(false);
		to.setText("#{ownerUsers}");
		...
		if (wizard.getSelected() == 3 && currentProcess != null) {
			...
			synchronized ( result ) {
				for (Object action[]: result) {
					JSONObject o = new JSONObject();
					o.put("name", action[0]);
					o.put("system", action[1]);
					o.put("description", action[2]);
					o.put("action", "E".equals(action[3]) ? Labels.getLabel("tools.disable-users.action1"):
						"D".equals(action[3]) ? Labels.getLabel("tools.disable-users.action2"):
						"R".equals(action[3]) ? Labels.getLabel("tools.disable-users.action3"): "");
