...
public class PamPolicyHandler extends FrameHandler {
	public PamPolicyHandler() throws InternalErrorException {
	}
	...
	public void checkCloseSession(Event event) {
		enableCheck(event, PamActionType.CLOSE_SESSION);
	}

	public void checkLockAccount(Event event) {
		enableCheck(event, PamActionType.LOCK_ACCOUNT);
	}

	public void checkIssue(Event event) {
		enableCheck(event, PamActionType.ISSUE);
	}

	public void checkNotify(Event event) {
		enableCheck(event, PamActionType.NOTIFY);
	}

	private void enableCheck(Event event, PamActionType action) {
		Component t = getFellow("actions");
		String[] data = (String[]) event.getData();
		boolean checked = "true".equals(data[0]); 
		List<PamActionType> actions = (List<PamActionType>) XPathUtils.eval(t, "actions");
		if (actions == null)
		...
		XPathUtils.setValue(t, "actions", new LinkedList<>(actions));
