...
Object.extend(Object.extend(zk.Tree.prototype, zk.Selectable.prototype), {
	 ...
	 */
	_showKids: function(row, C, B) {
	            var D = getZKAttr(row, "pitem");
	            do {
	                var r = row.nextSibling;
	                if ($tag(E) == "TR") {
	                    var A = getZKAttr(E, "gpitem");
	                    if (!D || D != A) {
	                        return row;
	                    }
	                    if (!B && (r.id.indexOf("!") > -1 || getZKAttr(E, "visible") == "true")) {
	                        if(C) {
	                        	r.classList.add("tree-hidden");
	                        	r.classList.remove("tree-displayed");
	                        }
	                        else {
	                        	r.classList.remove("tree-hidden");
	                        	r.classList.add("tree-displayed");
	                        }
	                    }
	                    E = this._showKids(E, C, C && (B || !zkTree.isOpen(E)))
	                }
	            } while (row = E)
	        },
